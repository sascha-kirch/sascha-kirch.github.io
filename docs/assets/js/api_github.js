...
function _updateGithubRepos(responseText) {
	...
	for (const element of sortedResponse) {
		...
		for (const topic of element.topics){
			topic_string += "<span class=\"tag is-small is-info is-light mt-1 mb-1\" >"+topic+"</span>"
		}
		...
		repo_info_string +=	"<div class=\"column is-full pb-3\">"
		...
		if (element.description != null){
			repo_info_string +=	"<div class=\"column is-full pt-3\">" + element.description + "</div>"
		}
		...
		repo_info_string +=	"<div class=\"column is-full pt-0 pb-0 mr-1\">" + topic_string + "</div>"
		repo_info_string +=	"</div>"
		repo_info_string +=	"</div>"
		repo_info_string +=	"</div>"
